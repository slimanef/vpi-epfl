  ...
  <xsl:template match="/">
    ...
    <site:view skin="home">
      ...
      <xsl:if test="contains($xslt.rights, 'modifier')">
        <site:menu>
          <button accesskey="e" title="modifier la nouvelle" onclick="javascript:window.location.href+='/modifier'">Modifier</button>
        </site:menu>
      </xsl:if>
      <site:title>
        <div id="title-homekp">
      ...
      <site:about>
        <div id="abouttitle">
          <xsl:apply-templates select="WebPage/Boxes/Box[4]/BoxTitle"/>
...
<!--          <xsl:apply-templates select="WebPage/Boxes/Box[4]/BoxContent"/>-->
          ...
          <a href="#video"><xsl:apply-templates select="WebPage/Boxes/Box[4]/BoxContent/BoxTitle[1]"/></a>
          <a href="#"><xsl:apply-templates select="WebPage/Boxes/Box[4]/BoxContent/BoxTitle[2]"/></a>
          <a href="#resources"><xsl:apply-templates select="WebPage/Boxes/Box[4]/BoxContent/BoxTitle[3]"/></a>
      ...
      <site:resources>
        ...
        <div id="abouttitle">
          ...
          <xsl:apply-templates select="WebPage/Boxes/Box[5]/*"/>
        </div>
          <div id="resources"> 
      ...
      <site:topics>
        <div id="abouttitle">
          <xsl:apply-templates select="WebPage/Boxes/Box[6]/*"/>
        </div>
         <div id="topics">
          <table class="table table-bordered">
            <thead>
         ...
         <div id="topics"> 
          ...
          <table class="table">
            ...
            <tbody  bgcolor="#FFF" localized="1">
                ...
                <xsl:apply-templates select="WebPage/TopLogos/Host[Photo/text()]/Photo"/> <xsl:text>&#160;&#160;&#160;&#160;&#160;</xsl:text>
      ...
      <site:sponsor1><span><xsl:value-of select="WebPage/Vocabulary/Sponsor1"/></span></site:sponsor1>
      <site:sponsor2><span><xsl:value-of select="WebPage/Vocabulary/Sponsor2"/></span></site:sponsor2>
    </site:view>
  ...
  <xsl:template match="Title">
    ...
    <b>  
    <a data-toggle="modal" href="directSearch/resources/{../Id}.modal" data-target="#resource-modal"><xsl:value-of select="."/></a>
    </b>
  </xsl:template>
  ...
  <xsl:template match="Name">
    ...
    <a data-toggle="modal" href="directSearch/topics/{../Id}.modal" data-target="#topic-modal"><xsl:value-of select="."/></a>
  ...
  <xsl:template match="Address">
    <div class="vcard">
      <p class="adr" style="margin-bottom:0">
        <span class="street-address"><xsl:value-of select="Street"/></span> - <xsl:apply-templates select="ExtendedAddress"/><br/>
        <span class="postal-code"><xsl:value-of select="Code"/></span> <span class="locality"><xsl:value-of select="City"/></span>
        <a href="contacts" class="btn btn-default btn-inverse" style="float:right; margin:5px">Plus</a>
      </p>
      <a class="email" href="mailto:{Organization/Email}"><xsl:value-of select="Organization/Email"/></a> 
    </div>
  </xsl:template>
  ...
  <xsl:template match="Project">
    <div class="col-xs-12">
      <div class="row headline">
        <div class="col-md-4 col-xs-12"><xsl:apply-templates select="Photo"/></div>
        <div class="col-md-8 col-xs-12">
          <h3 class="bold"><xsl:value-of select="PageTitle"/></h3>
          <p><xsl:value-of select="Summary"/></p>
          <!-- <p class="suite"><a href="projets/{@Key}"><xsl:call-template name="suite"/></a></p> -->
          <p class="suite"><a href="projets/{@Key}" class="btn btn-default btn-inverse">Plus</a></p>
        </div>
      </div>
    </div>
  </xsl:template>
...
<!--
  ...
  <xsl:template match="RefImage[Target]">
    <p>
      <a href="{Target}" target="_blank"><img class="refimg" src="{@Source}" alt="{Text}"/></a>
    </p>
  </xsl:template>
  ...
  <xsl:template match="NewsItem">
    <div class="col-xs-12">
      <div class="row headline">
        <div class="col-md-4 col-xs-12"><xsl:apply-templates select="Photo" mode="nouvelle"/></div>
        <div class="col-md-8 col-xs-12">
          <h3 class="bold"><xsl:value-of select="PageTitle"/></h3>
          <p><xsl:value-of select="Summary"/></p>
          <p class="suite"><a href="nouvelles/{@Key}" class="btn btn-default btn-inverse">Plus</a></p>
        </div>
      </div>
    </div>
  </xsl:template>
...
<!--  <xsl:template match="Photo" mode="nouvelle">
  ...
  <xsl:template name="contact">
    <xsl:variable name="base"><xsl:call-template name="static-base"/></xsl:variable>
    <p style="text-align:right">
      <a class="logolink" href="http://www.facebook.com/pages/AllianceTT/203013313052237#!/pages/AllianceTT/203013313052237?sk=wall" target="_blank">
      ...
      <a class="logolink" href="http://www.linkedin.com/company/alliance-tt?trk=hb_tab_compy_id_916723" target="_blank">
  ...
  <xsl:template name="static-base">
    <xsl:choose>
      <xsl:when test="contains($xslt.base-url,'/en/')"><xsl:value-of select="substring-before($xslt.base-url,'en/')"/></xsl:when>
      <xsl:otherwise><xsl:value-of select="$xslt.base-url"/></xsl:otherwise>
    </xsl:choose>
  </xsl:template>
  <xsl:template name="resource-modal">
    <!-- Modal -->
    <div id="resource-modal" class="modal hide fade more-infos" tabindex="-1" role="dialog" aria-labelledby="label-resource" aria-hidden="true">
  ...
  <xsl:template name="topic-modal">
    <!-- Modal -->
    <div id="topic-modal" class="modal hide fade more-infos" tabindex="-1" role="dialog" aria-labelledby="label-topic" aria-hidden="true">
      <div class="modal-header">
        <button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button>
