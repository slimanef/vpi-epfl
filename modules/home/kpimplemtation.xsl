...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template match="/">
    <site:view skin="home">
      ...
      <site:resources>
        <p id="video"><iframe width="560" height="315" src="https://www.youtube.com/embed/videoseries?list=PLcCGl9n-yR9gYJo5Pn2JTNTcS_JwHVaas" frameborder="0" allowfullscreen="allowfullscreen"></iframe></p>
        
      </site:resources>
      <site:topics>
        <div id="abouttitle">
          <p class="usermanual"><a href="sme" class="btn btn-default btn-inverse">HOST MANUAL</a></p>
        </div>
      </site:topics>
      ...
      <site:about>
        ...
        <div id="aboutcontent">
          ...
          <a href="#video">
            <xsl:apply-templates select="Root/WebPage/Boxes/Box[4]/BoxContent/BoxTitle[1]"/>
          </a>
          <a href="#">
            <xsl:apply-templates select="Root/WebPage/Boxes/Box[4]/BoxContent/BoxTitle[2]"/>
          </a>
          <a href="#resources">
            <xsl:apply-templates select="Root/WebPage/Boxes/Box[4]/BoxContent/BoxTitle[3]"/>
          </a>
  ...
  <xsl:template match="BlocContent">
    <div class="row">
      <div class="col-md-6">
        ...
        <xsl:apply-templates
          select="*[(local-name(.) != 'Index') and (local-name(.) != 'BlocTitle')]"/>
      </div>
  ...
  <xsl:template match="Link">
    <xsl:choose>
      <xsl:when test="starts-with(LinkRef, '/') and not(starts-with(LinkRef, '/static/'))">
        <a href="{concat($xslt.base-url, substring-after(LinkRef, '/'))}" target="_blank">
          <xsl:value-of select="LinkText"/>
        </a>
      </xsl:when>
      <xsl:when
        test="starts-with(LinkRef, 'http:') or starts-with(LinkRef, 'https:') or contains(LinkRef, 'static/kp/docs/')">
  ...
  <xsl:template match="Item">
    <li>
      <xsl:apply-templates select="Frag | Link | Fragment | Parag"/>
    </li>
  </xsl:template>
  ...
  <xsl:template match="Illustration[count(Stack/Element/Photo[. != '']) = 0]"> </xsl:template>
  ...
  <xsl:template match="Illustration">
    <div class="{normalize-space(concat('x-Illustration ', @Position))}">
      ...
      <xsl:choose>
        <xsl:when test="count(Stack/Element/Photo[. != '']) = 1">
          <div class="x-Element">
            <xsl:apply-templates select="Stack/Element"/>
          </div>
        </xsl:when>
        <xsl:otherwise>
          <table class="x-Stack">
            <tr>
              <xsl:apply-templates select="Stack/Element" mode="multi"/>
            </tr>
          </table>
        </xsl:otherwise>
      </xsl:choose>
  ...
  <xsl:template match="@Width">
    <xsl:attribute name="style">
      <xsl:value-of select="concat(concat('width:', .), 'px')"/>
    </xsl:attribute>
  </xsl:template>

  <xsl:template match="@LegendWidth">
    <xsl:value-of select="concat(concat(concat('width:', .), 'px'), ';')"/>
  </xsl:template>

  <xsl:template match="@LegendAlign">
    <xsl:value-of select="concat(concat('text-align:', .), ';')"/>
  </xsl:template>
  ...
  <xsl:template match="Element[LinkRef]">
    <xsl:variable name="prefix">
      <xsl:if test="$xslt.key != ''">
        <xsl:value-of select="concat($xslt.key, '/')"/>
      </xsl:if>
    </xsl:variable>
    <a target="_blank">
      <xsl:attribute name="href">
        <xsl:choose>
          ...
          <xsl:otherwise>
            <xsl:value-of select="concat('http://', LinkRef)"/>
          </xsl:otherwise>
        </xsl:choose>
      </xsl:attribute>
  ...
  <xsl:template match="Element">
    ...
    <a href="{$prefix}images/{substring-after(Photo, 'images/')}">
      <img class="x-Photo" src="{$prefix}vignettes/{substring-after(Photo, 'images/')}" alt="Photo"
      />
    </a>
  </xsl:template>
