...
declare function local:read( $lang as xs:string ) as element() {
  ...
  let $col-uri := replace(oppidum:path-to-ref-col(), 'pages', 'projets')
  let $sel-keys := doc($doc-uri)/WebPage/Projects/Project/string(@Key)
  let $handout := (doc($doc-uri)/WebPage/Projects/Project[Photo]/@Key)[1] (: Deprecated :)
  let $very-old := xs:date("1901-01-01")
  let $keys := distinct-values(collection($col-uri)/Project/@Key)
  return
    <site:view>
      <site:content>
        <xhtml:h1>Sélection de projets pour la page “accueil”</xhtml:h1>
        <xhtml:form action="." method="post">
          ...
          <xhtml:p>NOTE: n'oubliez pas de réenregistrer la liste si vous avec modifié le titre ou le résumé d'un projet</xhtml:p>
        </xhtml:form>
      </site:content>
    </site:view>
};
...
declare function local:update( $lang as xs:string ) {
  let $doc-uri := oppidum:path-to-ref()
  let $proj-col-uri := replace(oppidum:path-to-ref-col(), 'pages', 'projets')
  ...
  let $projects := 
    ...
    return
      <Project>
        ...
        if ($p/Index[. ne '']) then (: gives priority to square index image if existing :)
          <Photo>{ $p/Index/text() }</Photo>
        else
          $p/Photo
  ...
  return
    if (doc-available($doc-uri)) then (: sanity check :)
      ...
      return
        (
        update replace doc($doc-uri)/WebPage/Projects with $projects,
