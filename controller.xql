...
declare variable $actions := <actions error="models/error.xql">
  <action name="login" depth="0" epilogue="home"> <!-- may be GET or POST -->
    <model src="xcm:models/login.xql"/>
    <view src="xcm:views/login.xsl"/>
  </action>
  <action name="logout" depth="0">
    <model src="xcm:models/logout.xql"/>
  </action>
  <action name="GET" depth="2" trail="forms/*">
    <view src="xcm:modules/formulars/supergrid.xsl"/>
  </action>
</actions>;
...
let $mapping := fn:doc('/db/www/vpi-epfl/config/mapping.xml')/site
